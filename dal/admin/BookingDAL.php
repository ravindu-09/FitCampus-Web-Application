...
class BookingDAL {
    ...
    // Fetch all facilities for dropdowns and toggles
    public function getAllFacilities() {
        $stmtFac = $this->pdo->query("SELECT Facility_ID, Facility_Name FROM facility ORDER BY Facility_ID ASC");
        return $stmtFac->fetchAll(PDO::FETCH_ASSOC);
    }

    // Get facility capacity
    public function getFacilityCapacity($facility_id) {
        $stmtCap = $this->pdo->prepare("SELECT Capacity FROM facility WHERE Facility_ID = ?");
        $stmtCap->execute([$facility_id]);
        return $stmtCap->fetchColumn() ?: 50;
    }

    // Get total booked team size for a specific time slot
    public function getBookedSize($facility_id, $current_date, $time) {
    ...
    // Get slot specific booking details
    public function getSlotBookings($facility_id, $date, $time) {
    ...
    public function getBookingById($booking_id) {
        $stmt = $this->pdo->prepare("SELECT * FROM booking WHERE Booking_ID = ?");
        $stmt->execute([$booking_id]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }

    // Get approved occupants for conflict check
    public function getOccupants($facility_id, $reserve_date, $start_time) {
        $stmtOccupants = $this->pdo->prepare("
            ...
            AND (? >= b.Start_Time AND ? < b.End_Time)
        ");
        $stmtOccupants->execute([$facility_id, $reserve_date, $start_time, $start_time]);
        return $stmtOccupants->fetchAll(PDO::FETCH_ASSOC);
    }

    // Transaction for approving booking and cancelling conflicting ones
    public function approveAndOverride($booking_id, $admin_id, $cancel_ids, $cancel_reason) {
